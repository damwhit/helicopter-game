  ...
  start() {
    ...
    window.requestAnimationFrame(function gameLoop() {
      if (that.status === 'active') {
        ...
        if (that.copter.status === "shoot") {
          that.copter.drawBullet();
          that.copter.bullet.moveBullet();
        }
    ...
    copterShoot(that);
    liftCopterOnKeyDown(that);
    resetGravityOnKeyUp(that);
  }
}
...
function liftCopterOnKeyDown(that) {
...
function resetGravityOnKeyUp(that){
...
function copterShoot(that){
  window.addEventListener('keydown', function (event) {
    event.preventDefault();
    if (event.keyCode === 83 && that.copter.status === "shoot") {
      that.copter.shoot();
    }
  });
}
