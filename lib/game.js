...
class Game {
  ...
  randomizeColliders(colliders, copter) {
    colliders.forEach(function(collider){
      collider.draw().move();
      if (collider.x === -100) {
        collider.x = 800;
        if (collider instanceof Boundary){
  ...
  start() {
    ...
    window.requestAnimationFrame(function gameLoop() {
      if (that.status === 'active') {
        ...
        that.randomizeColliders(that.boundaries, that.copter);
        that.randomizeColliders(that.obstacles, that.copter);
